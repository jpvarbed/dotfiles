...
| Run is blocked by the trusted-folder / approval gate ("not running in a trusted folder") | The script already passes `--yolo --skip-trust` and exports `GEMINI_CLI_TRUST_WORKSPACE=true`. If invoking `gemini` by hand instead of the script, add the same flags (the `gy` alias bundles `gemini --yolo --skip-trust`). |
