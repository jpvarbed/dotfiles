...
- **Publish as an artifact:** load creds from bws (`bws-load`, then `ARTIFACT_API_BASE`/`ARTIFACT_API_KEY` via `bws secret list -o json | jq`), render the digest to a self-contained HTML file, then `bun ~/dev/artifact-studio-tools/cli/src/index.ts share <file.html> --slug dev-digest-<DATE> --visibility unlisted`. Capture the public URL.
